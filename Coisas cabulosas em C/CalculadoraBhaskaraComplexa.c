...
int main(){
    ...
    if(delta > 0){
        ...
        sprintf(&str1pt1[0], "%.2f", (-vars[1]) / (2 * vars[0]));         // <--- as quatro strings do diabo.
        sprintf(&str1pt2[0], "%.2f", (sqrt(fabs(delta)) / (2 * vars[0])));

        sprintf(&str2pt1[0], "%.2f", (-vars[1]) / (2 * vars[0]));
        sprintf(&str2pt2[0], "%.2f", + (sqrt(fabs(delta)) / (2 * vars[0])));
        
    }   
    //system("cls");
    //printf("%s\n%s\n%s\n%s", str1pt1, str1pt2, str2pt1, str2pt2);
    (delta == 0) ? printf("X = %.2f", ans1) : (delta > 0) ? printf("X = %.2f & %.2f", ans1, ans2) : printf("X = %s + %si & %s - %si", str1pt1, str1pt2, str2pt1, str2pt2);
}